...
export function canonicalReqId(raw: string | null | undefined): string | null {
	...
	const unlabelledToken = String(raw).trim()
		.replace(/^#\s*/, '')
		...
		.trim();
	if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(unlabelledToken)) return null;
	if ((unlabelledToken.match(/[0-9]/g)?.length ?? 0) < 5) return null;
	return unlabelledToken;
}
