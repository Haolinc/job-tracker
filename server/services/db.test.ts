...
describe('create / getAll', () => {
	...
	it('filters by search (company OR role, case-insensitive) and by status', async () => {
	...
	it('does not treat LIKE wildcards in search as wildcards', async () => {
...
describe('update / remove', () => {
	it('updates whitelisted fields and bumps updated_at; rejects unknown columns', async () => {
		const application = await db.create(baseData);
	...
	it('round-trips an emails array through update (the PATCH manual-edit path)', async () => {
		const application = await db.create(baseData);
	...
	it('remove returns true then false on repeat', async () => {
		const application = await db.create(baseData);
...
describe('updateWithEmail', () => {
	it('applies updates and appends the ref once — a duplicate messageId never double-records', async () => {
		const application = await db.create(baseData);
	...
	it('is a silent no-op for a missing id (matches the previous updateOne semantics)', async () => {
...
describe('findByCompanyFirstWord', () => {
	it('bounds the match so the next character is not alphanumeric', async () => {
	...
	it('matches a first word ending in punctuation (the "U.S. Bank" case)', async () => {
...
describe('findByCompanyDomain', () => {
	it('returns only exact-domain rows', async () => {
...
describe('synced emails', () => {
	it('markEmailSynced is first-write-wins; getSyncedMessageIds handles >500 ids (chunking)', async () => {
...
describe('clearAll', () => {
	it('wipes both tables and reports counts', async () => {
