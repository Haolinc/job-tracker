...
export async function findExisting(company: string, role: string | null, externalId: string | null, domain: string | null, isConfirmation: boolean, isFastApply: boolean, date: string): Promise<Application | undefined> {
	...
	const seenIds = new Set(byDomain.map(m => m.id));
    // When domain and name are both found, then dedup the overlapping record
	let companyApps = [...byDomain, ...byName.filter(m => !seenIds.has(m.id))];
	...
	if (externalId) {
		const sameReq = companyApps.find(m => m.external_id === externalId);
		...
		companyApps = companyApps.filter(m => !m.external_id);
		if (companyApps.length === 0) return undefined;
	}
	...
	if (incomingHasRole) {
		...
		if (isConfirmation) {
			const posting = postingMatches[0];
			// Pair a fast-apply with the company's own confirmation of the SAME application — but not one it
			// postdates (a later fast-apply is a re-application → new cycle).
			if (posting && (isFastApply || posting.fast_apply) && (!posting.date_applied || date <= posting.date_applied)) return posting;
			...
			const attachable = postingMatches.filter(m => !m.date_applied || m.date_applied <= date || m.awaiting_application);
		...
		const roleless = companyApps.filter(m => !m.role || m.role === 'Unknown Role');
	...
	if (isConfirmation) {
		...
		const awaiting = companyApps.filter(m => m.awaiting_application && (!m.date_applied || date <= m.date_applied));
		if (awaiting.length) return oldest(awaiting);
		const roled = companyApps.filter(m => m.role && m.role !== 'Unknown Role' && (!m.date_applied || date <= m.date_applied));
	...
	const predating = companyApps.filter(m => !m.date_applied || m.date_applied <= date || m.awaiting_application);
