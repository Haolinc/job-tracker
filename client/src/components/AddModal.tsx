...
export default function AddModal({ initial, onSave, onClose }: Props) {
	...
	const set = <K extends keyof ApplicationFormData>(field: K, value: ApplicationFormData[K]) =>
	...
	const canAddEmail = !!account && draftId !== null && !form.emails.some(emailRef => emailRef.messageId === draftId);
	...
	const removeEmail = (messageId: string) => set('emails', form.emails.filter(emailRef => emailRef.messageId !== messageId));
		...
		<div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
			<div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-md max-h-[92dvh] flex flex-col">
				...
				<form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 overflow-y-auto">
					<Field label="Company *">
						<input data-testid="field-company" value={form.company} onChange={event => set('company', event.target.value)}
							required className={inputCls} placeholder="Acme Corp" />
					</Field>
					<Field label="Role *">
						<input data-testid="field-role" value={form.role} onChange={event => set('role', event.target.value)}
					...
					{form.status === 'interview' && (
						<Field label="Interview Stage">
							<select data-testid="field-interview-step" value={form.interview_step} onChange={event => set('interview_step', event.target.value as InterviewStep | '')} className={inputCls}>
					...
					{form.status === 'rejected' && (
						<label className="flex items-start gap-2 text-sm text-gray-700 cursor-pointer select-none">
							<input
								...
								onChange={event => set('reached_interview', event.target.checked)}
					...
					<div className="grid grid-cols-2 gap-3">
						<Field label="Date Applied">
							<input data-testid="field-date-applied" type="date" value={form.date_applied} onChange={event => set('date_applied', event.target.value)} className={inputCls} />
						</Field>
						<Field label="Last Response">
							<input data-testid="field-last-activity" type="date" value={form.last_activity} onChange={event => set('last_activity', event.target.value)} className={inputCls} />
						</Field>
					</div>
					<Field label="Job URL">
						<input data-testid="field-job-url" value={form.job_url} onChange={event => set('job_url', event.target.value)}
							className={inputCls} placeholder="https://..." />
					</Field>
					<Field label="Job / Req ID">
						<input data-testid="field-external-id" value={form.external_id} onChange={event => set('external_id', event.target.value)}
					...
					<Field label="Tracked Emails">
						<div data-testid="field-emails" className="flex flex-col gap-2 rounded-lg border border-gray-200 p-3">
							<input data-testid="field-account" type="email" value={form.account} onChange={event => set('account', event.target.value)}
							...
							{form.emails.length > 0 && (
								<div className="flex flex-col gap-1 border-t border-gray-100 pt-2">
									...
									{form.emails.map((emailRef, emailIndex) => {
										...
										const originBadge = emailOriginBadge(emailRef);
										return (
											<div
											...
											>
												...
												<span data-testid="email-row-stage" data-fast-apply={emailRef.fast_apply ? 'true' : 'false'}
													className="px-1.5 py-0.5 rounded-full font-medium shrink-0 bg-gray-100 text-gray-600">
													{emailStageLabel(emailRef)}</span>
												<span className="flex-1 truncate text-gray-500" title={emailRef.messageId}>{emailRef.messageId}</span>
												{/* Shown here and nowhere else: the board is for scanning, this is where emails get curated. */}
												<span data-testid="email-origin-badge" data-origin={emailRef.origin ?? 'unknown'} title={originBadge.title}
													className={`px-1.5 py-0.5 rounded font-medium shrink-0 ${originBadge.cls}`}>{originBadge.label}</span>
												<button data-testid="email-row-remove" type="button" onClick={() => removeEmail(emailRef.messageId)}
							...
							<div className="flex items-center gap-2 border-t border-gray-100 pt-2">
								<select data-testid="email-draft-category" value={emailDraftCat} onChange={event => setEmailDraftCat(event.target.value as Status)}
									disabled={!account}
								...
								<input data-testid="email-draft-input" value={emailDraft} onChange={event => setEmailDraft(event.target.value)}
					...
					<Field label="Notes">
						<textarea data-testid="field-notes" value={form.notes} onChange={event => set('notes', event.target.value)}
